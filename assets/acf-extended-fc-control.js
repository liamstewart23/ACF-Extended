(function($) {
  ...
  // Layout: Edit Title
  model.events["focusout input.acfe-flexible-control-title"] =
    "acfeEditLayoutTitleToggle";
  ...
  model.acfeEditLayoutTitleToggle = function(e, $el) {
    ...
    if (flexible.has("acfeFlexibleModalEdition")) {
      var $hidden = $layout.find(
        "> .acfe-modal > .acfe-modal-wrapper > .acfe-modal-content > .acf-fields > .acf-field-acfe-flexible-layout-title > .acf-input > .acf-input-wrap > input"
      );
    } else {
      var $hidden = $layout.find(
        "> .acf-fields > .acf-field-acfe-flexible-layout-title > .acf-input > .acf-input-wrap > input"
      );
    }
    ...
    if ($hidden.length) {
      // Add Edit Title
      $hidden
        .addClass("acfe-flexible-control-title")
        .attr("data-acfe-flexible-control-title-input", 1)
        .insertAfter($handle);

      // Remove legacy field
      $layout
        .find("> .acf-fields > .acf-field-acfe-flexible-layout-title")
        .remove();
    }

    if ($layout.hasClass("acfe-flexible-title-edition")) {
      var $input = $title.find(
        "> input[data-acfe-flexible-control-title-input]"
      );
  ...
  // Layout: Edit Title
  model.events["click input.acfe-flexible-control-title"] =
    "acfeEditLayoutTitlePropagation";
  model.acfeEditLayoutTitlePropagation = function(e, $el) {
    e.stopPropagation();
  };

  // Layout: Edit Title Input
  model.events["input [data-acfe-flexible-control-title-input]"] =
    "acfeEditLayoutTitleInput";
  model.acfeEditLayoutTitleInput = function(e, $el) {
    // Vars
    var $layout = $el.closest(".layout");
    var $title = $layout.find(
      "> .acf-fc-layout-handle .acfe-layout-title .acfe-layout-title-text"
    );
  ...
  model.events["keypress [data-acfe-flexible-control-title-input]"] =
    "acfeEditLayoutTitleInputEnter";
  ...
  model.acfeCopyLayout = function(e, $el) {
    ...
    // Append Temp Input
    var $input = $(
      '<input type="text" style="clip:rect(0,0,0,0);clip-path:rect(0,0,0,0);position:absolute;" value="" />'
    ).appendTo($el);
    ...
    // Prompt
    else prompt("Copy the following layout data to your clipboard", data);
  ...
  model.acfeCopyLayouts = function() {
    ...
    // Prompt
    else prompt("Copy the following layouts data to your clipboard", data);
  ...
  // Flexible: Dropdown
  model.events['click [data-name="acfe-flexible-control-button"]'] =
    "acfeControl";
  ...
  model.acfeCleanLayouts = function($layout) {
    ...
    $layout.find(".acf-color-picker").each(function() {
      ...
      var $color_picker_proxy = $input
        .find(".wp-picker-container input.wp-color-picker")
        .clone();
  ...
  acf.addAction("new_field/type=flexible_content", function(flexible) {
    if (!flexible.has("acfeFlexibleCopyPaste")) return;
    ...
    if (flexible.has("acfeFlexibleStylisedButton")) {
      ...
      flexible.$el
        .find(
          "> .acf-input > .acf-flexible-content > .acfe-flexible-stylised-button > .acf-actions > .acf-button"
        )
        .after($dropdown);
    } else {
    ...
    /*
      ...
      flexible.$el
        .find(
          "> .acf-input > .acf-flexible-content > .acf-actions > .acf-button"
        )
        .after($dropdown);
    }
  });
  ...
  acf.addAction("acfe/flexible/layouts", function($layout, flexible) {
    ...
    if (flexible.has("acfeFlexibleCopyPaste")) {
      // Button: Copy
      $controls.prepend(
        '<a class="acf-icon small light acf-js-tooltip acfe-flexible-icon dashicons dashicons-category" href="#" title="Copy layout" data-acfe-flexible-control-copy="' +
    ...
    // Button: Clone
    $controls.prepend(
      '<a class="acf-icon small light acf-js-tooltip acfe-flexible-icon dashicons dashicons-admin-page" href="#" title="Clone layout" data-acfe-flexible-control-clone="' +
        $layout.attr("data-layout") +
        '"></a>'
    );
  });
